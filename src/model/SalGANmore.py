...
class SalGANdouble(nn.Module):

    """

    Memory explodes with this model. Impossible to train currently.


    """
    def __init__(self, seed_init, freeze = True, use_gpu=True):
        super(SalGANdouble,self).__init__()
        ...
        # select only convolutional layers
        first_convs = torch.nn.Sequential(*list(original_vgg16.features)[0:7])
        print(first_convs)
        print("encoder")

        encoder = torch.nn.Sequential(*list(original_vgg16.features)[0:30])
        print(encoder)
        ...
        self.input_size = 128
        self.hidden_size = 128
        self.GatesE = nn.Conv2d(in_channels = self.input_size + self.hidden_size, out_channels = 4 * self.hidden_size, kernel_size = (3, 3), padding = 1) #padding 1 to preserve HxW dimensions

        self.GatesD = nn.Conv2d(in_channels = self.input_size + self.hidden_size, out_channels = 4 * self.hidden_size, kernel_size = (3, 3), padding = 1) #padding 1 to preserve HxW dimensions
        ...
        torch.manual_seed(seed_init)
        for name, param in self.GatesE.named_parameters():
                if "weight" in name:
                    nn.init.xavier_normal_(param)
                elif "bias" in name:
                    nn.init.constant_(param, 0)
        ...
        for name, param in self.GatesD.named_parameters():
                if "weight" in name:
                    nn.init.xavier_normal_(param)
                elif "bias" in name:
                    nn.init.constant_(param, 0)
    ...
    def print_layers(self):
        ...
        print("Encoding ConvLSTM")
        for name, param in self.GatesE.named_parameters():
            print("For {} the requires_grad is {}".format(name, param.requires_grad))


        print("Decoding ConvLSTM")
        for name, param in self.GatesD.named_parameters():
            print("For {} the requires_grad is {}".format(name, param.requires_grad))
    ...
    def forward(self, input_, prev_state=(None, None)):

        x = self.salgan[0:7](input_)
        ...
        stacked_inputs = torch.cat((x, prev_hidden), 1)
        #print("stacked input size {}".format(stacked_inputs.size()))
        gates = self.GatesE(stacked_inputs)
        #print("stacked gates size {}".format(gates.size()))
        ...
        stacked_inputs = torch.cat((x, prev_hidden), 1)
        #print("stacked input size {}".format(stacked_inputs.size()))
        gates = self.GatesD(stacked_inputs)
        #print("stacked gates size {}".format(gates.size()))
        ...
        x = state_D[1]
        saliency_map = self.final_convs(x)

        return state_E, state_D, saliency_map



if __name__ == '__main__':
    model = SalGANdouble(seed_init=65)
